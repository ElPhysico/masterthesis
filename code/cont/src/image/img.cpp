...
void initPNG(string file)
{
	...
	double scale = (SIZE * 1.0) / svar.sysL;
...
{
	if (pos.x >= 0.5 * svar.sysL)
		pos.x -= svar.sysL;
	else if (pos.x <= -0.5 * svar.sysL)
		pos.x += svar.sysL;
	if (pos.y >= 0.5 * svar.sysL)
		pos.y -= svar.sysL;
	else if (pos.y <= -0.5 * svar.sysL)
		pos.y += svar.sysL;
